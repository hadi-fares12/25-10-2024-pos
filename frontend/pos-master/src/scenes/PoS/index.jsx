...
const handleBarcodeSearch = async () => {
  ...
  const foundItem = mealsCopy.find(
    (meal) =>
      (meal.ShowOnbarc === 'Y') &&
      (meal.ItemNo.toString() === barcode || meal.ItemName.toLowerCase() === barcode.toLowerCase())
  );

  if (foundItem) {
    setSearchedItem(foundItem);
    
    // Always add the found item to selectedMeals without checking if it exists
    setSelectedMeals(prevSelectedMeals => [
      ...prevSelectedMeals,
      { ...foundItem, quantity: 1, index: prevSelectedMeals.length } // Add with default quantity
    ]);
    
    // Clear the barcode input
    setBarcode('');
  } else {
    // If item not found in mealsCopy, fetch from API
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const apiFoundItem = filteredItems.find(
          (item) =>
            (item.ShowOnbarc === 'Y') &&
            (item.ItemNo.toString() === barcode || item.ItemName.toLowerCase() === barcode.toLowerCase())
        );

        if (apiFoundItem) {
          setSearchedItem(apiFoundItem);
          
          // Always add the found item to selectedMeals without checking if it exists
          setSelectedMeals(prevSelectedMeals => [
            ...prevSelectedMeals,
            { ...apiFoundItem, quantity: 1, index: prevSelectedMeals.length } // Add with default quantity
          ]);
